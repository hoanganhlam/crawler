...
exports.createUser = (req, res) => {
	let data = getBody(req, ['username', 'password', 'name']);
	return UserModel.findOne({username: data.username})
		.then(user => {
			if (user) return responseError(res, 'User has been existed', {messageCode: 'user_existed'}, 405);
			else return UserModel.create(data);
		})
		.then(created => {
